(function () {
    ...
    lessonCards.forEach(function (card) {
        ...
        const completionKey = "writing-speaking:" + lessonId + ":completed";
        const lastSlideKey = "writing-speaking:" + lessonId + ":last-slide";
        const isReady = card.dataset.lessonReady !== "false";
        const releaseDate = parseDateKey(card.dataset.lessonDate);
        const today = getLocalMidnight(new Date());
        const isDateOpen = !releaseDate || releaseDate.getTime() <= today.getTime();
        const isUnlocked = isReady && isDateOpen;
        const isComplete = isUnlocked && readStorage(completionKey) === "true";
        ...
        if (!isUnlocked) {
            ...
            if (badge) {
                badge.dataset.state = "soon";
                badge.textContent = "Locked";
            }
            if (status) {
                status.textContent = isReady && releaseDate
                    ? "Locked until " + formatDisplayDate(releaseDate) + "."
                    : "Locked. The title will appear when it opens.";
            }
            if (action) {
    ...
    const vocabProgramKey = "writing-speaking:vocab-program:start-date";
    const vocabDayCompletionPrefix = "writing-speaking:vocab-day:";
    ...
    function applyProgramDayState(link, options) {
        ...
        const contentReady = link.dataset.contentReady !== "false";
        const isUnlocked =
            contentReady && Number.isFinite(dayNumber) && dayNumber <= programState.activeDayNumber;
        const isCurrentDay =
        ...
        link.title = contentReady
            ? "Locked until " + formatDisplayDate(unlockDate)
            : "Locked until the words are uploaded";
